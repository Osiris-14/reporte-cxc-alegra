...
df_facturas.to_csv(out_facturas, index=False, encoding="utf-8-sig", float_format="%.2f")
print(f"CSV facturas: {out_facturas}  ({len(df_facturas)} filas)")

out_pagos = os.path.join(current_dir, "cxc_Pagos.csv")
df_pagos.to_csv(out_pagos, index=False, encoding="utf-8-sig", float_format="%.2f")
